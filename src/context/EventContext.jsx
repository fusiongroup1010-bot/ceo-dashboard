...
export const DEPARTMENTS = {
  all: [
    { id: 'hr-ga', name: 'HR-GA HQ', color: '#f43f5e', text: '#ffffff', accent: '#f43f5e' },
    { id: 'acc', name: 'ACC HQ', color: '#06b6d4', text: '#ffffff', accent: '#06b6d4' },
    { id: 'rnd', name: 'RNDSP', color: '#10b981', text: '#ffffff', accent: '#10b981' },
    { id: 'hcm-bm', name: 'HCM Branch Manager', color: '#d97706', text: '#ffffff', accent: '#d97706' },
    { id: 'hn-bm', name: 'HN Branch Manager', color: '#8b5cf6', text: '#ffffff', accent: '#8b5cf6' },
    { id: 'sale-online', name: 'Sale Online', color: '#ec4899', text: '#ffffff', accent: '#ec4899' },
    { id: 'purchasing', name: 'Purchasing', color: '#f97316', text: '#ffffff', accent: '#f97316' },
    { id: 'como', name: 'COMO', color: '#14b8a6', text: '#ffffff', accent: '#14b8a6' },
    { id: 'cpo', name: 'CPO', color: '#3b82f6', text: '#ffffff', accent: '#3b82f6' },
  ]
};
...
// Calculate task overdue status based on dueDate, dueTime range and duration hours
export const isTaskOverdue = (item) => {
  if (!item) return false;
  if (item.status === 'done' || item.status === 'in-progress') return false;
  if (!item.dueDate) return false;
  
  let dueDateTime;
  if (item.dueTime) {
    const startStr = `${item.dueDate}T${item.dueTime}`;
    const startDate = new Date(startStr);
    if (!isNaN(startDate.getTime())) {
      const durationHours = parseFloat(item.duration) || 1;
      dueDateTime = new Date(startDate.getTime() + durationHours * 60 * 60 * 1000);
    } else {
      dueDateTime = new Date(`${item.dueDate}T23:59:59`);
    }
  } else {
    dueDateTime = new Date(`${item.dueDate}T23:59:59`);
  }
...
export const EventProvider = ({ children }) => {
  const loadMergedState = (fbData) => {
    const fallbackFbStr = localStorage.getItem('ceo_last_known_fb_data');
    let baseData = fbData;
    ...
    if (!baseData && fallbackFbStr) {
        ...
        localStorage.setItem('ceo_last_known_fb_data', JSON.stringify(fbData));
    }

    const localStr = localStorage.getItem('ceo_local_changes');
  ...
  const saveChangeLocal = (id, objOrNull) => {
    ...
    localStorage.setItem('ceo_local_changes', JSON.stringify(localChanges));
  ...
  const syncPendingChangesToCloud = () => {
    ...
    keys.forEach(id => {
      ...
      if (change === null) {
        deleteDoc(doc(db, 'ceo_deadline_items', id)).catch(e => saveChangeLocal(id, null));
      } else {
        setDoc(doc(db, 'ceo_deadline_items', id), change, { merge: true }).catch(e => saveChangeLocal(id, change));
      }
    });
  };
  ...
  // Sync with Firestore
  useEffect(() => {
    const q = query(collection(db, 'ceo_deadline_items'), orderBy('dueDate', 'asc'));
  ...
  const addEvent = async (item) => {
    ...
    try { 
      await setDoc(doc(db, 'ceo_deadline_items', newId), newItem); 
  ...
  const updateEvent = async (updated) => {
    ...
    try { 
      await setDoc(doc(db, 'ceo_deadline_items', updated.id), updated, { merge: true }); 
  ...
  const deleteEvent = async (id) => {
    ...
    setCurrentEvent(null);
    try { 
      await deleteDoc(doc(db, 'ceo_deadline_items', id)); 
      clearChangeLocal(id);
    } catch (e) { console.warn(e); }
  };
  ...
  const changeStatus = async (id, status) => {
    setItems((prev) => {
      ...
      setDoc(doc(db, 'ceo_deadline_items', id), upd, { merge: true })
