...
export const NotifyProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, 'notifications_ceo_dashboard'),
  ...
  const sendNotification = async (notifData) => {
    ...
    await addDoc(collection(db, 'notifications_ceo_dashboard'), payload);
  };

  const markAsRead = async (id) => {
    if (!currentUser) return;
    const ref = doc(db, 'notifications_ceo_dashboard', id);
    await updateDoc(ref, {
  ...
  const deleteNotification = async (id) => {
    await deleteDoc(doc(db, 'notifications_ceo_dashboard', id));
  };
